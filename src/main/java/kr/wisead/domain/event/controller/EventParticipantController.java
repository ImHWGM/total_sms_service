...
public class EventParticipantController {
    ...
    public ApiResponse<EventParticipantResponse> createParticipant(
        ...
        EventParticipantResponse response = participantService.createParticipant(request, userDetails.getUsername());
    ...
    public ApiResponse<PageResponse<EventParticipantResponse>> getParticipants(
        ...
        ParticipantSearchRequest request = ParticipantSearchRequest.builder()
                .eventSeq(eventSeq)
                .keyword(keyword)
    ...
    public ApiResponse<EventCheckResponse> processAction(
            ...
            @AuthenticationPrincipal UserDetails userDetails) {
        EventCheckResponse response = checkService.processAction(eventSeq, request, userDetails.getUsername());
    ...
    public ResponseEntity<byte[]> downloadParticipantExcel(@PathVariable Integer eventSeq) {
        ...
        String fileName = "참가자목록_" + eventSeq + "_" + LocalDate.now().format(DateTimeFormatter.BASIC_ISO_DATE) + ".xlsx";
    ...
    public ResponseEntity<byte[]> downloadStatisticsExcel(@PathVariable Integer eventSeq) {
        ...
        String fileName = "행사통계_" + eventSeq + "_" + LocalDate.now().format(DateTimeFormatter.BASIC_ISO_DATE) + ".xlsx";
        String encodedFileName = URLEncoder.encode(fileName, StandardCharsets.UTF_8);
