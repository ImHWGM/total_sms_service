...
public class EventService {
  ...
  public EventResponse createEvent(
    ...
    // JSON 방식: 임시 폴더의 파일들을 eventSeq 폴더로 이동
    if (tempId != null) {
      boolean moved = fileStorageService.moveSurveyTempToEvent(tempId, eventSeq);
      if (moved) {
        log.info("임시 이미지 폴더 이동 완료 - tempId: {}, eventSeq: {}", tempId, eventSeq);
        ...
        if (request.getEventEndImg() != null && !request.getEventEndImg().isEmpty()) {
          String newEndImgPath = convertTempPathToEventPath(request.getEventEndImg(), eventSeqStr);
  ...
  private String extractTempIdFromPath(String path) {
    ...
    // temp_ 패턴 찾기
    int tempIndex = path.indexOf("temp_");
    if (tempIndex == -1) {
      return null;
    }
    // temp_ 이후의 폴더명 추출
    String afterTemp = path.substring(tempIndex + 5);
    int separatorIndex = afterTemp.indexOf('/');
    if (separatorIndex == -1) {
      separatorIndex = afterTemp.indexOf('\\');
    }
    if (separatorIndex == -1) {
      return afterTemp;
    }
    return afterTemp.substring(0, separatorIndex);
  }
  ...
  private void saveQuestionsWithImages(
    ...
    for (QuestionRequest qReq : questions) {
      ...
      if (qReq.getQuestionImg() != null && !qReq.getQuestionImg().isEmpty()) {
        String newQuestionImgPath = convertTempPathToEventPath(qReq.getQuestionImg(), eventSeqStr);
        question.setQuestionImg(newQuestionImgPath);
        log.info("문항 이미지 경로 변환 - questionOrder: {}, path: {}", questionOrder, newQuestionImgPath);
      }
      ...
      if (qReq.getItems() != null && !qReq.getItems().isEmpty()) {
        ...
        for (ItemRequest iReq : qReq.getItems()) {
          ...
          if (iReq.getItemImg() != null && !iReq.getItemImg().isEmpty()) {
            ...
            item.setItemImg(newItemImgPath);
            log.info(
                "항목 이미지 경로 변환 - questionOrder: {}, itemOrder: {}, path: {}",
                questionOrder,
                currentItemOrder,
                newItemImgPath);
          }
  ...
  private void saveQuestionsWithImages(
    ...
    for (QuestionRequest qReq : questions) {
      ...
      if (questionImgFile != null) {
        ...
        log.info(
            "문항 이미지 저장 완료 - eventSeq: {}, questionSeq: {}, path: {}",
            eventSeq,
            question.getQuestionSeq(),
            questionImgPath);
      }
      ...
      if (qReq.getItems() != null && !qReq.getItems().isEmpty()) {
        ...
        for (ItemRequest iReq : qReq.getItems()) {
          ...
          if (itemImgFile != null) {
            ...
            log.info(
                "항목 이미지 저장 완료 - eventSeq: {}, questionSeq: {}, itemSeq: {}, path: {}",
                eventSeq,
                question.getQuestionSeq(),
                item.getItemSeq(),
                itemImgPath);
  ...
  /** 범용인증코드 엑셀 업로드 - 엑셀 파일에서 EVENT_SEQ(A), AUTH_CODE(B) 컬럼을 읽어서 등록 */
  @Transactional
  public Map<String, Object> uploadAuthKeyExcel(MultipartFile file, String regId) {
    ...
    log.info(
        "범용인증코드 엑셀 업로드 완료 - 전체: {}, 성공: {}, 실패: {}", excelContent.size(), successCount, failCount);
  ...
  private void addSurveySummarySheet(
    ...
    createLabelValueRow(
        row5, 2, "응답자(응답률)", completedCount + "명(" + responseRate + ")", labelStyle, valueStyle);
    ...
    createLabelValueRow(
        row6, 2, "미응답자(미응답률)", absenteesCount + "명(" + absenteeRate + ")", labelStyle, valueStyle);
  ...
  /** 시트2: 전체_응답결과 (문항별 통계) */
  private void addSurveyResponsesSheet(
      SXSSFWorkbook workbook,
      Integer eventSeq,
      List<SurveyQuestion> questions,
      List<SurveyItem> allItems) {
  ...
  private void addSurveyParticipantsSheet(
      SXSSFWorkbook workbook,
      SurveyMaster event,
      List<SurveyQuestion> questions,
    ...
    if (!questions.isEmpty()) {
      ...
      if (questions.size() > 1) {
        sheet.addMergedRegion(
            new CellRangeAddress(0, 0, startCol, startCol + questions.size() - 1));
      }
    }
  ...
  private CellStyle createExcelStyle(
      SXSSFWorkbook workbook, int fontSize, boolean bold, int r, int g, int b) {
  ...
  /** "##" 구분자 이전의 첫 번째 값 추출 (구분자 없으면 원본 반환) */
  ...
  /** 현장등록 QR 코드 생성 */
  @Transactional
  public String generateOnsiteRegistrationQrCode(Integer eventSeq, String userId) {
    SurveyMaster event =
        surveyMasterMapper
            .selectByEventSeq(eventSeq)
            .orElseThrow(
    ...
    // 현장등록 페이지 URL로 QR 코드 생성
    String qrContents = wiseadUrl + "/event/register/" + event.getEventCode();
    ...
    if (fileName == null || fileName.isEmpty()) {
      throw new BusinessException(ErrorCode.INTERNAL_ERROR, "QR 코드 생성에 실패했습니다.");
    }

    String qrCodeImgPath = "/qrcode/" + fileName;

    // SURVEY_MASTER QR 정보 업데이트
    surveyMasterMapper.updateQrCode(eventSeq, "Y", qrCodeImgPath);

    log.info(
        "현장등록 QR 코드 생성 완료 - eventSeq: {}, eventCode: {}, path: {}",
        eventSeq,
        event.getEventCode(),
        qrCodeImgPath);
  ...
  private String generateQrCodeImage(String authCodeUrl) {
    try {
      ...
      // 다른 이미지들과 동일하게 상대 경로 반환 (예: /qrcode/xxx.png)
