...
/**
 * 일반 문자(Multi Message) 발송 Controller
 * SMS/LMS/MMS 각각 다른 단가 적용
 */
@Slf4j
@RestController
...
public class MultiMessageController {
    ...
    public ApiResponse<MultiMessageResponse> sendMessages(
            ...
            @RequestBody MultiMessageRequest request) {

        String regId = userDetails.getUsername();
        MultiMessageResponse response = multiMessageService.sendDirectMessage(request, regId);
        return ApiResponse.success(response);
    }

    /**
     * 일반 문자 발송 (MMS 이미지 포함 - Multipart)
     * POST /api/multi/send/mms
     */
    ...
    /**
     * receivers JSON 문자열 파싱
     * 형식: [{"phone":"01012345678","repChar01":"홍길동"},...]
     */
    private List<MultiMessageRequest.ReceiverInfo> parseReceivers(String receiversJson) {
        ...
        try {
            ...
            List<java.util.Map<String, String>> list = mapper.readValue(receiversJson,
            ...
            for (java.util.Map<String, String> item : list) {
                receivers.add(MultiMessageRequest.ReceiverInfo.builder()
