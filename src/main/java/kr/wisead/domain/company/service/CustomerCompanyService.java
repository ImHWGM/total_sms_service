...
public class CustomerCompanyService {
  ...
  /** 고객사 목록 조회 (페이징) */
  public PageResponse<CustomerCompanyResponse> getCompanyList(
      CustomerCompanySearchRequest request) {
  ...
  public int toggleCompanySelection(CustomerCompanyRequest request, String operatorId) {
    String exists =
        customerCompanyMapper.selectCompanyExists(
            request.getUserId(), request.getSelectedUserId(), request.getCustCompName());

    CustomerCompany company =
        CustomerCompany.builder()
  ...
  public boolean enrollCompanies(CustomerCompanyRequest request, String operatorId) {
    ...
    if (request.getSelectedUserIds().size() != request.getCustCompNames().size()) {
      ...
      throw new BusinessException(
          ErrorCode.INVALID_INPUT_VALUE, "선택된 사용자 ID 목록과 고객사명 목록의 개수가 일치해야 합니다.");
    }

    // 유효한 데이터만 필터링하여 CustomerCompany 객체 리스트 생성
