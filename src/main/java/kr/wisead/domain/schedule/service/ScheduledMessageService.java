...
public class ScheduledMessageService {
  ...
  public PageResponse<ScheduledMessageResponse> getScheduledMessages(
  ...
  /** 예약 시간 변경 */
  @Transactional
  public void rescheduleMessageGroup(
      String userId, String msgType, LocalDateTime insertTime, LocalDateTime newScheduleTime) {
    // 10분 이내 예약은 불가
    ...
    int updated =
        scheduledMessageMapper.updateGroupRequestTime(userId, msgType, insertTime, newScheduleTime);
  ...
  private BigDecimal calculateRefundAmount(
      String msgType, int messageCount, BalanceResponse balance) {
    BigDecimal unitPrice =
        switch (msgType) {
          case "S" ->
              balance.getSmsPrice() != null
                  ? balance.getSmsPrice()
                  : balance.getSubtractUnitPrice();
          case "L" ->
              balance.getLmsPrice() != null
                  ? balance.getLmsPrice()
                  : balance.getSubtractUnitPrice();
          case "M" ->
              balance.getMmsPrice() != null
                  ? balance.getMmsPrice()
                  : balance.getSubtractUnitPrice();
