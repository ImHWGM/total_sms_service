 ...
 *
 * <p>Test Scenarios: - TC2.2.1: Single resend success (use original content) - TC2.2.2: Single
 * resend success (new content) - TC2.2.3: Single resend failure - user not found - TC2.2.4: Batch
 * resend success - TC2.2.5: Batch resend partial success - TC2.2.6: Duplicate resend success -
 * TC2.2.7: Resend with insufficient balance (PENDING - requires balance integration) - TC2.2.8:
...
class MessageResendIntegrationTest {
  ...
  @MockitoBean private MessageSendService messageSendService;
  ...
  void TC2_2_1_resendSingle_UseOriginal_Success() throws Exception {
    ...
    // Verify
    verify(messageSendService, times(1))
        .resendSurveyMessage(
            eq(123),
            isNull(),
            isNull(),
    ...
    when(messageSendService.resendSurveyMessage(
        ...
        .thenThrow(
            new kr.wisead.common.exception.BusinessException(
                kr.wisead.common.response.ErrorCode.RESOURCE_NOT_FOUND, "Survey user not found"));
  ...
  void TC2_2_8_resend_NoAuth_Returns401() throws Exception {
    ...
    verify(messageSendService, never())
        .resendSurveyMessage(
            anyInt(),
            anyString(),
            anyString(),
  ...
  void TC2_2_11_resendDuplicate_WithReplacementChars_Success() throws Exception {
    ...
    // Verify - check that the service was called with request containing repChar
    // values
    verify(messageSendService)
        .resendToDuplicates(
            argThat(
                req ->
