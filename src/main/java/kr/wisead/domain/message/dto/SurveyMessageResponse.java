...
public class SurveyMessageResponse {
     ...
     * 성공 응답
     */
    public static SurveyMessageResponse success(int successCount, List<Integer> mseqList,
     ...
     * 부분 성공 응답
     */
    public static SurveyMessageResponse partial(int successCount, int failCount,
                                                 int duplicateCount, List<String> failedPhones,
                                                 List<Integer> mseqList, String txGroupId) {
        return SurveyMessageResponse.builder()
                .success(true)
                .message(String.format("설문 문자 발송 완료 (성공: %d건, 실패: %d건, 중복: %d건)",
