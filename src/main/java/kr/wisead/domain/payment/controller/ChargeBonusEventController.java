...
public class ChargeBonusEventController {
  ...
  /** 이벤트 목록 조회 GET /api/admin/charge-bonus-event */
  @GetMapping
  ...
  /** 이벤트 상세 조회 GET /api/admin/charge-bonus-event/{eventSeq} */
  @GetMapping("/{eventSeq}")
  ...
  /** 이벤트 생성 POST /api/admin/charge-bonus-event */
  @PostMapping
  public ApiResponse<ChargeBonusEventResponse> createEvent(
      @CurrentUser JwtPrincipal user, @Valid @RequestBody ChargeBonusEventRequest request) {
    ChargeBonusEventResponse event = chargeBonusEventService.createEvent(request, user.userId());
    return ApiResponse.success(event);
  }

  /** 이벤트 수정 PUT /api/admin/charge-bonus-event/{eventSeq} */
  @PutMapping("/{eventSeq}")
  public ApiResponse<ChargeBonusEventResponse> updateEvent(
    ...
    ChargeBonusEventResponse event =
        chargeBonusEventService.updateEvent(eventSeq, request, user.userId());
    return ApiResponse.success(event);
  }

  /** 이벤트 상태 변경 (활성화/비활성화) PATCH /api/admin/charge-bonus-event/{eventSeq}/status */
  @PatchMapping("/{eventSeq}/status")
  public ApiResponse<Void> updateEventStatus(
      @PathVariable Long eventSeq,
      @RequestParam String status,
      @CurrentUser JwtPrincipal user) {
    chargeBonusEventService.updateEventStatus(eventSeq, status, user.userId());
    return ApiResponse.success(null);
  }

  /** 이벤트 삭제 DELETE /api/admin/charge-bonus-event/{eventSeq} */
  @DeleteMapping("/{eventSeq}")
  public ApiResponse<Void> deleteEvent(
      @PathVariable Long eventSeq, @CurrentUser JwtPrincipal user) {
    chargeBonusEventService.deleteEvent(eventSeq, user.userId());
    return ApiResponse.success(null);
  }

  /** 현재 활성 이벤트 목록 조회 (충전 금액 기준) GET /api/admin/charge-bonus-event/active?chargeAmount=50000 */
  @GetMapping("/active")
