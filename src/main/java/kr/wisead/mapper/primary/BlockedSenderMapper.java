...
public interface BlockedSenderMapper {
  ...
  List<BlockedSender> selectBlockedSendersWithPaging(
      @Param("dtmf1") String dtmf1, @Param("offset") int offset, @Param("limit") int limit);
  ...
  List<String> selectBlockedNumbers(
      @Param("dtmf1") String dtmf1, @Param("aniList") List<String> aniList);

  /** 수신거부 검색 조회 (페이징) */
  List<BlockedSender> selectBlockedSendersWithSearch(
      @Param("storeCodes") List<String> storeCodes,
      @Param("senderId") String senderId,
      @Param("encryptedAni") String encryptedAni,
      @Param("offset") int offset,
      @Param("limit") int limit);

  /** 수신거부 검색 건수 */
  int countBlockedSendersWithSearch(
      @Param("storeCodes") List<String> storeCodes,
      @Param("senderId") String senderId,
      @Param("encryptedAni") String encryptedAni);
}
