...
public class ShortUrlUtils {
  ...
  /** 설문 URL 패턴: /auth/ 경로가 포함된 URL */
  private static final String AUTH_URL_PATTERN =
      "(https?://[^\\s]+/auth/(?:qrcode/)?[A-Za-z0-9]+(?:/[A-Za-z0-9]+)?)(?![A-Za-z0-9/])";

  /** QR코드 URL 패턴: /qrcode/ 경로가 포함된 URL */
  private static final String QR_URL_PATTERN =
      "(https?://[^\\s]+/qrcode/[A-Za-z0-9]+(?:/[A-Za-z0-9]+)?)(?![A-Za-z0-9/])";

  /**
   * 텍스트 내 설문 URL을 모두 단축 URL로 변환
   *
   * @param text 원본 텍스트
   * @param baseUrl 설문 서비스 기본 URL (사용되지 않음, 제네릭 패턴 사용)
   * @return URL이 단축된 텍스트
   */
  public static String shortenUrlsInText(String text, String baseUrl) {
    return shortenUrlsByPattern(text, AUTH_URL_PATTERN, "URL");
  }
  ...
  /**
   * 텍스트 내 QR코드 URL을 모두 단축 URL로 변환 /qrcode/ 패턴 포함 URL을 단축
   *
   * @param text 원본 텍스트
   * @return URL이 단축된 텍스트
   */
  public static String shortenQrUrlsInText(String text) {
    return shortenUrlsByPattern(text, QR_URL_PATTERN, "QR URL");
  }

  /**
   * 패턴에 매칭되는 URL을 모두 단축 URL로 변환
   *
   * @param text 원본 텍스트
   * @param regex URL 패턴 정규식
   * @param logPrefix 로그 접두사
   * @return URL이 단축된 텍스트
   */
  private static String shortenUrlsByPattern(String text, String regex, String logPrefix) {
    ...
    try {
      ...
      while (matcher.find()) {
        ...
        log.debug("{} 단축: {} -> {}", logPrefix, longUrl, shortUrl);
      ...
      log.warn("텍스트 내 {} 단축 실패 - error: {}", logPrefix, e.getMessage());
      return text;
    }
  }
}
