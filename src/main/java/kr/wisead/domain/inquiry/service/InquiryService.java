...
public class InquiryService {
  ...
  public InquiryResponse submitInquiry(InquiryRequest request) {
    ...
    log.info(
        "문의 등록 완료: inquiryId={}, company={}", inquiry.getInquiryId(), inquiry.getCompanyName());
  ...
  /** 문의 목록 조회 (페이징) */
  public PageResponse<InquiryResponse> getInquiryList(
    ...
    List<InquiryResponse> responses = list.stream().map(InquiryResponse::from).toList();
  ...
  public InquiryResponse answerInquiry(
      Long inquiryId, InquiryAnswerRequest request, String answeredBy) {
    // 권한 체크: A레벨만 답변 가능
