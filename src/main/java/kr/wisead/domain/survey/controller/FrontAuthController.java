...
public class FrontAuthController {
    ...
    /**
     * QR 코드 접근 시 사용자 생성
     * - 인증이 필요 없는 설문에서 QR 접근 시 사용자 자동 생성
     */
    @PostMapping("/qr/user")
    ...
    /**
     * 가상 키패드 데이터 생성
     * - 주민번호 등 민감정보 입력 시 RSA 암호화용 공개키 발급
     */
    @GetMapping("/keypad")
    ...
    /**
     * KCP 인증 시작 데이터 생성
     * - 모바일 앱에서 WebView로 KCP 인증 페이지 호출 전 필요한 데이터 생성
     */
    @GetMapping("/kcp/init")
    ...
    /**
     * KCP 인증 결과 처리
     * - KCP 인증 완료 후 암호화된 데이터를 복호화하여 사용자 정보 반환
     */
    @PostMapping("/kcp/result")
    public ApiResponse<KcpAuthResponse> processKcpAuthResult(
            @RequestBody KcpAuthRequest request) {
