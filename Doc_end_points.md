...
| POST | `/api/history/send/download` | downloadSendHistory | 발송 이력 엑셀 다운로드 | O | Query: `startDate`, `endDate`, `type`, `keyword`, `sendResult` (success/failure); Body: `{ password, reason }` |
