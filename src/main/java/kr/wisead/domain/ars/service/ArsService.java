...
/** ARS 수신거부 서비스 - ARS 시스템 연동 전용 로직 - 차단 번호 관리는 BlockedNumberService 사용 */
@Slf4j
@Service
...
public class ArsService {
  ...
  /** 전화번호로 발송자의 상점코드 조회 (자동등록형) 최근 3개월 테이블에서 검색 (SMS DB) */
    ...
    for (int i = 0; i < maxAttempts; i++) {
      try {
        ...
        if (userId != null) {
          ...
          return userMapper.findByUserId(userId).map(user -> user.getStoreCode()).orElse(null);
        }
      } catch (Exception e) {
  ...
  /** 수신거부 등록 (ARS용) */
  @Transactional
  public boolean registerBlockedSender(
      String ani, String storeCode, String menuName, String tTime) {
  ...
  public PageResponse<BlockedSenderResponse> searchBlockedSenders(
    ...
    return blockedNumberService.searchBlockedNumbers(
