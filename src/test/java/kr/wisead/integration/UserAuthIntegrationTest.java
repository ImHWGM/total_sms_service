 ...
 *
 * <p>테스트 시나리오: 1. 사용자가 회원가입을 신청한다 (상태: 미승인) 2. 관리자가 회원을 승인한다 (상태: 미승인 -> 승인) 3. 미승인 상태에서 로그인 시도 시
 * 실패 4. 승인 후 로그인 성공
 */
@SpringBootTest
...
class UserAuthIntegrationTest {
  ...
  void checkUserId_Duplicate_ReturnTrue() throws Exception {
    ...
    // When & Then
    mockMvc
        .perform(get("/api/auth/check-userid").param("userId", TEST_USER_ID))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
    ...
    when(authService.login(any(LoginRequest.class)))
        .thenThrow(
            new kr.wisead.common.exception.BusinessException(
                kr.wisead.common.response.ErrorCode.ACCOUNT_DISABLED, "승인 대기 중인 계정입니다."));
  ...
  void updateStatus_AdminApprove_Success() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/users/{userId}/status", TEST_USER_ID)
  ...
  void login_WrongPassword_Fail() throws Exception {
    // Given: 잘못된 비밀번호
    LoginRequest request =
        LoginRequest.builder().userId(TEST_USER_ID).userPass("wrongPassword1!").build();

    // Mock: 로그인 실패 예외
    when(authService.login(any(LoginRequest.class)))
        .thenThrow(
            new kr.wisead.common.exception.BusinessException(
                kr.wisead.common.response.ErrorCode.LOGIN_FAILED, "아이디 또는 비밀번호가 일치하지 않습니다."));
  ...
  void login_LockedAccount_Fail() throws Exception {
    ...
    when(authService.login(any(LoginRequest.class)))
        .thenThrow(
            new kr.wisead.common.exception.BusinessException(
                kr.wisead.common.response.ErrorCode.ACCOUNT_LOCKED, "로그인 실패 횟수 초과로 계정이 잠겼습니다."));
  ...
  void unlockAccount_Admin_Success() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/users/{userId}/unlock", TEST_USER_ID)
                .header("Authorization", "Bearer " + adminToken))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true));
  ...
  void TC1_1_1_login_ValidCredentials_ReturnsJwtToken() throws Exception {
    ...
    LoginResponse loginResponse =
        LoginResponse.builder()
            .accessToken(
                "eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJ0ZXN0dXNlcjAxIiwiZXhwIjoxNzM2NzYyMDAwfQ.test")
            .refreshToken(
  ...
  /**
   * TC1.1.3: 5회 실패 후 계정 잠금 -> 423 에러 (LOCKED) Note: Spring에서는 403 Forbidden을 사용하며,
   * ErrorCode.ACCOUNT_LOCKED를 반환
   */
  @Test
  @Order(103)
  ...
  /** TC1.1.4: 휴면 계정 로그인 시도 -> 적절한 에러 메시지 Note: 현재 시스템에서는 ACCOUNT_DISABLED로 처리됨 */
  @Test
  @Order(104)
  ...
  /** TC1.1.5: 비밀번호 만료 계정 -> 변경 요청 응답 Note: Legacy 시스템에서는 result: 180으로 처리됨. 현재 시스템에서 구현 필요. */
  @Test
  @Order(105)
   ...
   *
   * <p>Scenario: 사용자가 올바른 정보로 회원가입을 신청한다 Expected: 회원가입 성공 메시지 반환
   */
  @Test
  @Order(121)
   ...
   *
   * <p>Scenario: 이미 사용 중인 아이디로 회원가입을 시도한다 Expected: 409 Conflict 에러와 중복 메시지 반환
   */
  @Test
  @Order(122)
  ...
  void TC1_2_2_signUp_DuplicateUserId_Returns409() throws Exception {
    ...
    doThrow(
            new kr.wisead.common.exception.BusinessException(
                kr.wisead.common.response.ErrorCode.DUPLICATE_EMAIL, "이미 사용 중인 아이디입니다."))
        .when(authService)
   ...
   *
   * <p>Scenario: 올바르지 않은 이메일 형식으로 회원가입을 시도한다 Expected: 400 Bad Request 에러와 유효성 검사 메시지 반환
   */
  @Test
  @Order(123)
   ...
   *
   * <p>Scenario: 필수 필드(기업명)를 누락하고 회원가입을 시도한다 Expected: 400 Bad Request 에러와 필수 필드 메시지 반환
   */
  @Test
  @Order(124)
   ...
   *
   * <p>Scenario: 비밀번호와 비밀번호 확인이 일치하지 않는다 Expected: 400 Bad Request 에러와 PASSWORD_MISMATCH 코드 반환
   */
  @Test
  @Order(125)
   ...
   *
   * <p>Legacy 시스템: POST /qvey/signUpIdChk.do -> result: 0 New 시스템: GET /api/auth/check-userid ->
   * duplicate: false
   */
  @Test
  @Order(131)
  ...
  void TC1_3_1_checkUserId_Available_ReturnsFalse() throws Exception {
    ...
    // When & Then
    mockMvc
        .perform(get("/api/auth/check-userid").param("userId", availableUserId))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
   ...
   *
   * <p>Legacy 시스템: POST /qvey/signUpIdChk.do -> result: 1 (or more) New 시스템: GET
   * /api/auth/check-userid -> duplicate: true
   */
  @Test
  @Order(132)
   ...
   *
   * <p>New 시스템 신규 기능: JWT Refresh Token으로 새 토큰 발급 Legacy 시스템: Session-based 인증으로 해당 기능 없음
   */
  @Test
  @Order(141)
  ...
  void TC1_4_1_refreshToken_ValidToken_ReturnsNewTokens() throws Exception {
    ...
    TokenRefreshRequest request =
        TokenRefreshRequest.builder().refreshToken(validRefreshToken).build();
   ...
   *
   * <p>ErrorCode.INVALID_TOKEN (A002) 반환
   */
  @Test
  @Order(142)
  ...
  void TC1_4_2_refreshToken_ExpiredToken_Returns401() throws Exception {
    ...
    TokenRefreshRequest request =
        TokenRefreshRequest.builder().refreshToken("expired-refresh-token").build();

    // Mock: 만료된 토큰 예외
    when(authService.refreshToken(anyString()))
        .thenThrow(
            new kr.wisead.common.exception.BusinessException(
                kr.wisead.common.response.ErrorCode.INVALID_TOKEN, "유효하지 않은 Refresh Token입니다."));
   ...
   *
   * <p>JWT 형식이 아닌 문자열 전송시 INVALID_TOKEN (A002) 반환
   */
  @Test
  @Order(143)
  ...
  void TC1_4_3_refreshToken_MalformedToken_Returns401() throws Exception {
    // Given: 잘못된 형식의 token
    TokenRefreshRequest request =
        TokenRefreshRequest.builder().refreshToken("not-a-valid-jwt-token-format").build();
  ...
  /** TC1.4.4: 빈 refresh token -> 400 에러 (Validation) @Valid 어노테이션에 의해 @NotBlank 검증 실패 */
  @Test
  @Order(144)
  ...
  void TC1_4_4_refreshToken_EmptyToken_Returns400() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/auth/refresh").contentType(MediaType.APPLICATION_JSON).content(requestJson))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.success").value(false));
   ...
   *
   * <p>토큰은 유효하지만 계정이 비활성화된 경우 ACCOUNT_DISABLED (A006) 반환
   */
  @Test
  @Order(145)
  ...
  void TC1_4_5_refreshToken_DisabledAccount_Returns403() throws Exception {
    ...
    TokenRefreshRequest request =
        TokenRefreshRequest.builder().refreshToken("valid-token-but-account-disabled").build();

    // Mock: 계정 비활성화 예외
    when(authService.refreshToken(anyString()))
        .thenThrow(
            new kr.wisead.common.exception.BusinessException(
                kr.wisead.common.response.ErrorCode.ACCOUNT_DISABLED, "비활성화된 계정입니다."));
   ...
   *
   * <p>Legacy 시스템: Session 기반, 복잡한 데이터 조합 (balance, companies 포함) New 시스템: JWT 기반, 사용자 정보만 반환
   * (RESTful 분리)
   *
   * <p>주요 차이점: - Legacy: 92 lines controller, Model + JSP View - New: 4 lines controller, JSON
   * Response - Legacy: Manual session check - New: @AuthenticationPrincipal 자동 주입
   */
  @Test
  @Order(151)
  ...
  void TC1_5_1_getMyInfo_AuthenticatedUser_ReturnsUserInfo() throws Exception {
    ...
    // Mock: UserService 응답
    kr.wisead.domain.user.dto.UserResponse expectedResponse =
        kr.wisead.domain.user.dto.UserResponse.builder()
            .seq(1)
            .userId(TEST_USER_ID)
            .corpName("테스트기업")
            .corpAddr("서울시 강남구")
    ...
    // When & Then
    mockMvc
        .perform(get("/api/users/me").header("Authorization", "Bearer " + validAccessToken))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
   ...
   *
   * <p>JWT 토큰 없이 /api/users/me 접근 시 401 Unauthorized
   *
   * <p>Legacy 시스템: Session 없으면 로그인 페이지로 리다이렉트 New 시스템: 401 HTTP Status 반환 (RESTful)
   */
  @Test
  @Order(152)
   ...
   *
   * <p>New 시스템 2단계 흐름: 1. /api/users/find-id/request - 정보 검증 + 이메일 인증코드 발송 2.
   * /api/users/find-id/verify - 인증코드 확인 + 마스킹된 아이디 반환
   *
   * <p>Legacy 시스템: 단일 요청으로 아이디 목록 반환 (보안 취약)
   */
  @Test
  @Order(171)
  ...
  void TC1_7_1_findId_ValidInfo_ReturnsEmailAndMaskedId() throws Exception {
    ...
    kr.wisead.domain.user.dto.FindIdRequest findIdRequest =
        kr.wisead.domain.user.dto.FindIdRequest.builder()
            .corpName("테스트기업")
            .person("홍길동")
            .phone("010-2345-6789")
            .build();
    ...
    kr.wisead.domain.user.dto.FindIdResponse step1Response =
        kr.wisead.domain.user.dto.FindIdResponse.requestSuccess("te***@example.com");
    when(userService.requestFindId(any(kr.wisead.domain.user.dto.FindIdRequest.class)))
    ...
    kr.wisead.domain.email.dto.EmailVerificationRequest verifyRequest =
        kr.wisead.domain.email.dto.EmailVerificationRequest.builder()
            .email("test@example.com")
            .code("123456")
            .build();

    // Mock: Step 2 응답 - 마스킹된 아이디 반환
    kr.wisead.domain.user.dto.FindIdResponse step2Response =
        kr.wisead.domain.user.dto.FindIdResponse.verifySuccess("tes*****");
    when(userService.verifyAndGetUserId(eq("test@example.com"), eq("123456")))
   ...
   *
   * <p>resultCode: 2 반환 (계정 없음) Legacy 시스템: 빈 ArrayList 반환 New 시스템: 구조화된 실패 응답
   */
  @Test
  @Order(172)
  ...
  void TC1_7_2_findId_UserNotFound_ReturnsNotFoundResponse() throws Exception {
    ...
    // Mock: 사용자 없음 응답
    kr.wisead.domain.user.dto.FindIdResponse notFoundResponse =
        kr.wisead.domain.user.dto.FindIdResponse.accountNotFound();
    when(userService.requestFindId(any(kr.wisead.domain.user.dto.FindIdRequest.class)))
   ...
   *
   * <p>resultCode: 3 반환 (인증 실패) Legacy 시스템: 인증 단계 없음 New 시스템: 2단계 이메일 인증으로 보안 강화
   */
  @Test
  @Order(173)
  ...
  void TC1_7_3_findId_InvalidCode_ReturnsVerificationFailed() throws Exception {
    ...
    // Mock: 인증 실패 응답
    kr.wisead.domain.user.dto.FindIdResponse failedResponse =
        kr.wisead.domain.user.dto.FindIdResponse.verificationFailed();
    when(userService.verifyAndGetUserId(eq("test@example.com"), eq("wrong_code")))
  ...
  /**
   * TC1.7.4: 아이디 찾기 - 필수 필드 누락 @Valid 어노테이션에 의해 400 Bad Request Legacy 시스템: 검증 없이 DB 쿼리 (결과 없음 반환)
   ...
   *
   * <p>POST /api/users/find-id (email + person) 2단계 없이 바로 마스킹된 아이디 반환
   */
  @Test
  @Order(175)
   ...
   *
   * <p>Legacy 시스템: 임시 비밀번호 직접 반환 (보안 취약) New 시스템: 토큰 기반 이메일 링크 발송 (보안 강화)
   *
   * <p>주요 차이점: - Legacy: newPw를 응답에 직접 포함 - New: 마스킹된 이메일만 반환, 재설정 링크는 이메일로 발송
   */
  @Test
  @Order(181)
  ...
  void TC1_8_1_findPassword_ValidInfo_SendsResetEmail() throws Exception {
    // Given: 비밀번호 찾기 요청
    kr.wisead.domain.user.dto.FindPasswordRequest request =
        new kr.wisead.domain.user.dto.FindPasswordRequest();
    ...
    verify(userService, times(1))
        .findPassword(any(kr.wisead.domain.user.dto.FindPasswordRequest.class));
  }

  /**
   * TC1.8.2: 비밀번호 찾기 - 계정 정보 없음
   *
   * <p>resultCode: 2 반환 (계정 없음) Legacy 시스템: result: 2, msg: "계정 정보가 없습니다." New 시스템: resultCode: 2,
   ...
   *
   * <p>resultCode: 3 반환 (힌트 불일치) Legacy 시스템: result: 3, msg: "비밀번호 힌트를 확인해주세요." New 시스템:
   * resultCode: 3, message: "비밀번호 힌트가 일치하지 않습니다."
   */
  @Test
  @Order(183)
  ...
  /** TC1.8.4: 비밀번호 찾기 - 필수 필드 누락 @Valid 어노테이션에 의해 400 Bad Request */
  @Test
  @Order(184)
  ...
  void TC1_8_4_findPassword_MissingFields_Returns400() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/users/find-pw").contentType(MediaType.APPLICATION_JSON).content(requestJson))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.success").value(false));
   ...
   *
   * <p>New 시스템 전용 기능: 토큰 기반 비밀번호 재설정 Legacy 시스템: 임시 비밀번호 직접 반환 방식 (이 기능 없음)
   */
  @Test
  @Order(185)
  ...
  void TC1_8_5_passwordReset_ValidToken_ChangesPassword() throws Exception {
    ...
    doNothing()
        .when(userService)
        .resetPasswordWithToken(eq("valid-uuid-token-12345"), eq("NewPassword123!"));
  ...
  void TC1_8_6_passwordReset_ExpiredToken_Returns400() throws Exception {
    ...
    doThrow(
        ...
        .when(userService)
        .resetPasswordWithToken(eq("expired-uuid-token"), eq("NewPassword123!"));
  ...
  void TC1_8_7_passwordReset_UsedToken_Returns400() throws Exception {
    ...
    doThrow(
            new kr.wisead.common.exception.BusinessException(
                kr.wisead.common.response.ErrorCode.INVALID_INPUT_VALUE, "이미 사용된 토큰입니다."))
        .when(userService)
        .resetPasswordWithToken(eq("used-uuid-token"), eq("NewPassword123!"));
   ...
   *
   * <p>프론트엔드에서 비밀번호 재설정 폼 표시 전 토큰 검증
   */
  @Test
  @Order(188)
  ...
  void TC1_8_8_validateToken_ValidToken_ReturnsValid() throws Exception {
    ...
    // Mock: 토큰 유효성 검증 성공
    Map<String, Object> validationResult = Map.of("valid", true, "userId", "tes*****");
    when(userService.validateResetToken(eq(validToken))).thenReturn(validationResult);

    // When & Then
    mockMvc
        .perform(get("/api/users/password/reset-validate").param("token", validToken))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
  ...
  void TC1_8_9_validateToken_InvalidToken_Returns400() throws Exception {
    ...
    when(userService.validateResetToken(eq(invalidToken)))
        .thenThrow(
            new kr.wisead.common.exception.BusinessException(
                kr.wisead.common.response.ErrorCode.INVALID_INPUT_VALUE, "유효하지 않은 토큰입니다."));

    // When & Then
    mockMvc
        .perform(get("/api/users/password/reset-validate").param("token", invalidToken))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.success").value(false))
  ...
  void TC1_9_1_unlockAccount_AdminUser_Success() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/users/{userId}/unlock", lockedUserId)
                .header("Authorization", "Bearer " + adminToken)
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
  ...
  void TC1_9_2_unlockAccount_NonAdminUser_Forbidden() throws Exception {
    ...
    String userToken = jwtTokenProvider.createAccessToken(userAuth);
  ...
  void TC1_9_3_unlockAccount_NoAuthentication_Unauthorized() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/users/{userId}/unlock", lockedUserId).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isUnauthorized());
  ...
  void TC1_9_4_unlockAccount_UserNotFound_Returns404() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/users/{userId}/unlock", nonExistentUserId)
                .header("Authorization", "Bearer " + adminToken)
                .contentType(MediaType.APPLICATION_JSON))
  ...
  void TC1_9_5_unlockAccount_AlreadyUnlocked_StillSuccess() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/users/{userId}/unlock", activeUserId)
                .header("Authorization", "Bearer " + adminToken)
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true));
