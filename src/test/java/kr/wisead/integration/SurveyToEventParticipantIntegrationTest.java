 ...
 * 설문 응답 → 행사 참여 통합 테스트
 *
 * <p>테스트 시나리오 A: 사전 설문 응답 후 현장 체크인 1. 행사(설문) 생성 2. 참가자 등록 (SURVEY_USER) 3. 사전 설문 발송 및 응답 4. 현장에서 QR
 * 체크인 5. 경품/기념품 수령
 *
 * <p>테스트 시나리오 B: 설문 미응답 상태에서 현장 체크인 1. 행사에 등록된 참가자가 설문 미응답 상태 2. 현장에서 QR 체크인 가능 3. 경품 수령 (관리자 판단에
 * 따라)
 */
@SpringBootTest
...
class SurveyToEventParticipantIntegrationTest {
  ...
  @MockitoBean private SurveyUserService surveyUserService;
  ...
  void createSurveyUsers_Success() throws Exception {
    // Given: 설문 대상자 등록
    SurveyUserRequest request =
        SurveyUserRequest.builder().eventSeq(TEST_EVENT_SEQ).userPhone("01023456789").build();

    SurveyUserResponse mockResponse =
  ...
  void submitSurvey_BeforeEvent_Success() throws Exception {
    ...
    doNothing()
        .when(surveyService)
        .submitSurvey(eq(TEST_EVENT_SEQ), any(SurveySubmitRequest.class));

    // When & Then: 인증 없이 설문 응답
    mockMvc
        .perform(
            post("/api/survey/{eventSeq}/submit", TEST_EVENT_SEQ)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true));
  ...
  void checkIn_Respondent_Success() throws Exception {
    ...
    mockMvc
        .perform(
            post(
                    "/api/events/{eventSeq}/check/{checkCode}",
                    TEST_EVENT_SEQ,
                    TEST_CHECK_CODE_RESPONDENT)
        ...
        EventParticipantResponse.builder()
            ...
            .qrCodeUrl(
                "http://localhost:8080/event/"
                    + TEST_EVENT_SEQ
                    + "/check/"
                    + TEST_CHECK_CODE_NON_RESPONDENT)
            .build();
  ...
  void createParticipant_WithoutAuth_Fail() throws Exception {
    ...
    EventParticipantRequest request =
        EventParticipantRequest.builder().userName("테스트").userPhone("010-1111-2222").build();
  ...
  void checkIn_NoAuthRequired_Success() throws Exception {
    ...
    EventCheckResponse mockResponse =
        EventCheckResponse.builder()
            .action("CHECK_IN")
            .actionName("입장")
            .participant(EventCheckResponse.ParticipantInfo.builder().name("테스트").build())
            .message("입장 처리되었습니다.")
            .build();
    ...
    mockMvc
        .perform(post("/api/events/{eventSeq}/check/{checkCode}", TEST_EVENT_SEQ, "any_check_code"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true));
  }
  ...
  void getParticipantStatus_NoAuthRequired_Success() throws Exception {
    ...
    ParticipantStatusResponse mockResponse =
        ParticipantStatusResponse.builder()
            .participant(ParticipantStatusResponse.ParticipantInfo.builder().name("테스트").build())
            .actions(List.of())
            .build();
    ...
    mockMvc
        .perform(get("/api/events/{eventSeq}/check/{checkCode}", TEST_EVENT_SEQ, "any_check_code"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true));
  }
}
