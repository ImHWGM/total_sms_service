...
public class InquiryController {
    ...
    /**
     * 문의 목록 조회 (관리자용)
     * GET /api/inquiry/list?status=PENDING&keyword=검색어&page=1&size=10
     */
    @GetMapping("/list")
    public ApiResponse<PageResponse<InquiryResponse>> getInquiryList(
        ...
        PageResponse<InquiryResponse> response = inquiryService.getInquiryList(status, keyword, page, size);
        return ApiResponse.success(response);
    }
    ...
    public ApiResponse<InquiryResponse> answerInquiry(
        ...
        String answeredBy = userDetails.getUsername();
        InquiryResponse response = inquiryService.answerInquiry(inquiryId, request, answeredBy);
        return ApiResponse.success(response);
    }

    /**
     * 문의 상태 변경 (관리자용)
     * PUT /api/inquiry/{inquiryId}/status
     */
    ...
    /**
     * 대기중 문의 개수 조회 (관리자 대시보드용)
     * GET /api/inquiry/pending/count
     */
