 ...
 * 테스트 시나리오:
 * 1. 관리자가 행사를 생성한다
 * 2. 액션 유형을 설정한다 (입장, 경품수령, 기념품수령)
 * 3. 참가자를 등록한다
 * 4. 참가자가 QR 스캔으로 체크인한다 (인증 불필요)
 * 5. 관리자가 경품/기념품 수령 처리를 한다 (관리자 비밀번호 필요)
 * 6. 명찰을 출력한다
 */
@SpringBootTest
...
class EventParticipantWorkflowIntegrationTest {
    ...
    void getActionTypes_Success() throws Exception {
        // Given: 기본 액션 유형 목록
        List<EventActionTypeResponse> actionTypes = List.of(
        ...
        // When & Then
        mockMvc.perform(get("/api/events/{eventSeq}/action-types", TEST_EVENT_SEQ)
                        .header("Authorization", "Bearer " + userToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
    ...
    void createPrizeActionType_Success() throws Exception {
        ...
        EventActionTypeRequest request = EventActionTypeRequest.builder()
                .actionCode("PRIZE")
                .actionName("경품 수령")
                .requireAdminAuth("Y")
                .allowMultiple("N")
        ...
        EventActionTypeResponse mockResponse = EventActionTypeResponse.builder()
        ...
        // When & Then
        mockMvc.perform(post("/api/events/{eventSeq}/action-types", TEST_EVENT_SEQ)
                        .header("Authorization", "Bearer " + userToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isOk())
    ...
    void createParticipant_Success() throws Exception {
        ...
        EventParticipantResponse mockResponse = EventParticipantResponse.builder()
                ...
                .participantType("일반")
                .nametagPrinted("N")
                .qrCodeUrl("http://localhost:8080/event/check/" + TEST_CHECK_CODE)
                .regDate(LocalDateTime.now())
                .build();
        ...
        verify(participantService, times(1)).createParticipant(any(EventParticipantRequest.class), anyString());
    }
    ...
    void getParticipants_Success() throws Exception {
        // Given: 참가자 목록
        List<EventParticipantResponse> participants = List.of(
        ...
        // When & Then
        mockMvc.perform(get("/api/events/{eventSeq}/participants", TEST_EVENT_SEQ)
                        .header("Authorization", "Bearer " + userToken)
                        .param("page", "1")
                        .param("size", "20"))
                .andExpect(status().isOk())
    ...
    void getParticipantByCheckCode_NoAuth_Success() throws Exception {
        ...
        when(participantService.getParticipantStatusByCheckCode(TEST_CHECK_CODE))
                .thenReturn(mockResponse);

        // When & Then: 인증 없이 접근 가능
        mockMvc.perform(get("/api/events/check/{checkCode}", TEST_CHECK_CODE))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
    ...
    void checkIn_NoAuth_Success() throws Exception {
        ...
        when(checkService.checkIn(eq(TEST_CHECK_CODE), any()))
                .thenReturn(mockResponse);

        // When & Then: 인증 없이 체크인 가능
        mockMvc.perform(post("/api/events/check/{checkCode}", TEST_CHECK_CODE)
        ...
        verify(checkService, times(1)).checkIn(eq(TEST_CHECK_CODE), any());
    }
    ...
    void getParticipantStatus_AfterCheckIn() throws Exception {
        ...
        when(participantService.getParticipantStatus(TEST_PARTICIPANT_SEQ))
                .thenReturn(mockResponse);

        // When & Then
        mockMvc.perform(get("/api/events/{eventSeq}/participants/{seq}/status", TEST_EVENT_SEQ, TEST_PARTICIPANT_SEQ)
                        .header("Authorization", "Bearer " + userToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.actions[0].actionCode").value("CHECK_IN"))
    ...
    void getNametagData_Success() throws Exception {
        ...
        // When & Then
        mockMvc.perform(get("/api/events/{eventSeq}/participants/{seq}/nametag", TEST_EVENT_SEQ, TEST_PARTICIPANT_SEQ)
                        .header("Authorization", "Bearer " + userToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
    ...
    void recordNametagPrint_Success() throws Exception {
        // Given: 명찰 출력 요청
        NametagPrintRequest request = NametagPrintRequest.builder()
        ...
        // When & Then
        mockMvc.perform(post("/api/events/{eventSeq}/participants/{seq}/nametag/print", TEST_EVENT_SEQ, TEST_PARTICIPANT_SEQ)
                        .header("Authorization", "Bearer " + userToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isOk())
    ...
    void processAction_Prize_Success() throws Exception {
        ...
        verify(checkService, times(1)).processAction(eq(TEST_EVENT_SEQ), any(EventCheckRequest.class), anyString());
    }
    ...
    void checkIn_InvalidCheckCode_Fail() throws Exception {
        ...
        when(checkService.checkIn(eq(invalidCheckCode), any()))
                .thenThrow(new BusinessException(ErrorCode.RESOURCE_NOT_FOUND, "참가자 정보를 찾을 수 없습니다."));

        // When & Then
        mockMvc.perform(post("/api/events/check/{checkCode}", invalidCheckCode))
    ...
    void deleteParticipant_Success() throws Exception {
        ...
        // When & Then
        mockMvc.perform(delete("/api/events/{eventSeq}/participants/{seq}", TEST_EVENT_SEQ, TEST_PARTICIPANT_SEQ)
                        .header("Authorization", "Bearer " + userToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
