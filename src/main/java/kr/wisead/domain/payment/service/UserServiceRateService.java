...
/** 사용자별 서비스 요금 Service - userSeq(Integer) 기반으로 동작 */
@Slf4j
@Service
...
public class UserServiceRateService {
  ...
  /**
   * 사용자별 요금 설정 (userSeq 기반) - vatIncluded=false면 rate * 1.1 계산 - endDate 없으면 기존 요금 종료 후 새 요금 영구 적용
   * - endDate 있으면 기간 특별요금으로 등록 (기존 요금 유지)
   */
  @Transactional
  public void updateUserRates(Integer userSeq, UserServiceRateRequest request) {
    ...
    for (UserServiceRateRequest.RateEntry entry : request.rates()) {
      ...
      if (endDate == null) {
        ...
        terminateCurrentPermanentRate(userSeq, entry.serviceId(), startDate.minusDays(1));
      }
      // 기간 특별요금: 기존 요금 그대로 유지

      // 새 요금 등록
      UserServiceRate newRate =
          UserServiceRate.builder()
              .userSeq(userSeq)
      ...
      log.info(
          "사용자 요금 설정: userSeq={}, serviceId={}, rate={}, startDate={}, endDate={}",
          userSeq,
          entry.serviceId(),
          rate,
          startDate,
  ...
  /** 현재 영구 요금 종료 처리 */
  private void terminateCurrentPermanentRate(Integer userSeq, String serviceId, LocalDate endDate) {
    Optional<UserServiceRate> currentRate =
        userServiceRateMapper.selectLatestPermanentRate(userSeq, serviceId);
    if (currentRate.isPresent()) {
      userServiceRateMapper.updateEndDate(currentRate.get().getSeq(), endDate);
      log.info("기존 영구 요금 종료: userSeq={}, serviceId={}, endDate={}", userSeq, serviceId, endDate);
    }
  }

  /** 사용자별 현재 적용 요금 조회 (userSeq 기반) - 우선순위: 기간 특별요금 > 사용자 기본요금 > 표준요금 */
  @Transactional(readOnly = true)
  public UserServiceRateResponse getUserRates(Integer userSeq) {
    LocalDate today = LocalDate.now();
    ...
    // 사용자의 모든 활성 요금 조회
    List<UserServiceRate> userRates = userServiceRateMapper.selectAllActiveRates(userSeq, today);
    Map<String, UserServiceRate> userRateMap =
        userRates.stream()
            .collect(
                Collectors.toMap(
    ...
    for (StandardRate stdRate : standardRates) {
      BigDecimal standardRateWithVat =
          stdRate.getServiceRate().multiply(VAT_RATE).setScale(1, RoundingMode.HALF_UP);

      UserServiceRate userRate = userRateMap.get(stdRate.getServiceId());
      boolean hasCustomRate = userRate != null;

      UserServiceRateResponse.ServiceRateInfo info =
          new UserServiceRateResponse.ServiceRateInfo(
  ...
  /** 특정 서비스의 현재 적용 요금 조회 (차감 시 사용, userSeq 기반) 우선순위: 기간 특별요금 → 사용자 기본요금 → 표준요금 */
  @Transactional(readOnly = true)
  public BigDecimal getEffectiveRate(Integer userSeq, String serviceId) {
    LocalDate today = LocalDate.now();

    // 1. 기간 특별요금 조회
    Optional<UserServiceRate> periodRate =
        userServiceRateMapper.selectPeriodRate(userSeq, serviceId, today);
    if (periodRate.isPresent()) {
      log.debug(
          "기간 특별요금 적용: userSeq={}, serviceId={}, rate={}",
          userSeq,
          serviceId,
          periodRate.get().getRate());
      return periodRate.get().getRate();
    }

    // 2. 사용자 기본요금(영구) 조회
    Optional<UserServiceRate> permanentRate =
        userServiceRateMapper.selectLatestPermanentRate(userSeq, serviceId);
    if (permanentRate.isPresent() && permanentRate.get().isActive(today)) {
      log.debug(
          "사용자 기본요금 적용: userSeq={}, serviceId={}, rate={}",
          userSeq,
          serviceId,
          permanentRate.get().getRate());
    ...
    if (standardRate != null) {
      ...
      log.debug("표준요금 적용: userSeq={}, serviceId={}, rate={}", userSeq, serviceId, rateWithVat);
      return rateWithVat;
    }

    log.warn("요금 정보 없음: userSeq={}, serviceId={}", userSeq, serviceId);
