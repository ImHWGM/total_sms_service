 ...
 * SMS 인증 Controller.
 *
 * <p>회원가입(사전 인증) 흐름 전용. 4개 인증 메서드 모두 {@link SmsVerificationService}(key=phone)에 라우팅된다. 로그인/2FA
...
public class SmsVerificationController {
  ...
  private final SmsVerificationService smsAuthService;
  ...
  public ApiResponse<Void> sendVerificationCode(@Valid @RequestBody SmsVerificationRequest request) {
    smsAuthService.sendVerificationCode(
        request.getPhoneNumber(), SmsVerificationService.PURPOSE_SIGNUP);
  ...
  public ApiResponse<Boolean> verifyCode(@Valid @RequestBody SmsVerificationRequest request) {
    boolean verified =
        smsAuthService.verifyCode(
            request.getPhoneNumber(), request.getCode(), SmsVerificationService.PURPOSE_SIGNUP);
  ...
  public ApiResponse<VerificationStatus> getVerificationStatus(
    ...
    VerificationStatus status =
        smsAuthService.getVerificationStatus(phoneNumber, SmsVerificationService.PURPOSE_SIGNUP);
