...
public class BillingExcelService {
     ...
     * 과금 통계 Excel 생성
     */
    public byte[] generateBillingExcel(BillingStatsSearchRequest request, String userId, List<String> targetUserIds) {
    ...
    private void addUsageSummarySheet(SXSSFWorkbook workbook, BillingStatsSearchRequest request) {
        ...
        periodRow.createCell(1).setCellValue("조회기간: " + request.getStartDate() + " ~ " + request.getEndDate());
        ...
        // 데이터 (서비스 타입별 통계)
        List<ServiceTypeBillingStatsResponse> stats = billingStatisticsService.getBillingStatsByServiceType(request);
        int rowIdx = 5;
        for (ServiceTypeBillingStatsResponse stat : stats) {
            Row row = sheet.createRow(rowIdx++);
            row.createCell(1).setCellValue(stat.getServiceTypeName() != null ? stat.getServiceTypeName() : "");
            row.createCell(2).setCellValue(stat.getTransactionCount());

            Cell amountCell = row.createCell(3);
            amountCell.setCellValue(stat.getTotalAmount() != null ? stat.getTotalAmount().doubleValue() : 0);
    ...
    private void addMessageDetailSheet(SXSSFWorkbook workbook, BillingStatsSearchRequest request) {
        ...
        // 헤더
        String[] headers = {"사용자 ID", "사용자명", "SMS 총건수", "SMS 성공", "LMS 총건수", "LMS 성공", "MMS 총건수", "MMS 성공", "합계"};
        Row headerRow = sheet.createRow(4);
        for (int i = 0; i < headers.length; i++) {
            Cell cell = headerRow.createCell(i + 1);
            cell.setCellValue(headers[i]);
        ...
        // 데이터
        StatsSearchRequest statsRequest = StatsSearchRequest.builder()
     ...
     * 4. 사용자별 요금 내역 시트
     */
    private void addUserHistorySheets(SXSSFWorkbook workbook, BillingStatsSearchRequest request, List<String> targetUserIds) {
        ...
        for (String userId : targetUserIds) {
            // 사용자별 거래 내역 조회
            List<Transaction> transactions = transactionMapper.selectByDateRange(
                    userId, startDateTime, endDateTime);

            // 사용 내역이 있는 경우만 시트 생성
            boolean hasUsableData = transactions.stream()
                    .anyMatch(tx -> !"CHARGE".equals(tx.getTxType()));
            ...
            periodRow.createCell(0).setCellValue("조회기간: " + request.getStartDate() + " ~ " + request.getEndDate());
            ...
            for (Transaction tx : transactions) {
                ...
                row.createCell(0).setCellValue(tx.getRegDate() != null ?
                        tx.getRegDate().format(DATETIME_FORMATTER) : "");
