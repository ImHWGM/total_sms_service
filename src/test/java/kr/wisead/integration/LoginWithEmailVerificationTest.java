 ...
 *
 * 테스트 시나리오:
 * 1. 그날 처음 로그인 시도 -> 이메일 인증코드 발송 필요
 * 2. 이메일 인증코드 발송
 * 3. 이메일 인증코드 검증
 * 4. 인증 완료 후 로그인 성공
 * 5. 같은 날 재로그인 -> 이메일 인증 없이 바로 로그인
 */
@SpringBootTest
...
class LoginWithEmailVerificationTest {
    ...
    void getVerificationStatus_CodeSent() throws Exception {
        // Given: 인증코드가 발송된 상태
        EmailAuthService.VerificationStatus status =
                new EmailAuthService.VerificationStatus(true, 240, 0, 5);

        when(emailAuthService.getVerificationStatus(TEST_EMAIL)).thenReturn(status);

        // When & Then
        mockMvc.perform(get("/api/email/verification/status")
